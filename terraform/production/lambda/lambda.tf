...
variable "ETL_LAMBDA_FUNCTION_NAME" {
  description = "ETL lambda function name (on Prod environment)."
  type        = string
}

variable "LAMBDA_EXECUTION_ROLE_NAME" {
  description = "Execution role name to run Lambda functions (on Prod environment)."
  type        = string
}

variable "ETL_LAMBDA_IMAGE_URI" {
  description = "Docker image that will be ran by the ETL lambda function (on Prod environment)."
  type        = string
}

variable "ETL_LAMBDA_LOG_GROUP" {
  description = "Log group where the logg messages will be saved (on Prod environment)."
  type        = string
} 

variable "ETL_LAMBDA_FUNCTION_MEMORY_SIZE" {
  description = "Memory assigned to the lambda function (on Prod environment)."
  type        = string
}

variable "ETL_LAMBDA_FUNCTION_TIMEOUT" {
  description = "Amount of time in seconds that the lambda function is allowed to run (on Prod environment)."
  type        = string
}
...
resource "aws_iam_role" "lambda_execution_role_prod" {
...
# LAMBDA FUNCTION
resource "aws_lambda_function" "etl_lambda_prod" {
  ...
  # Execution role
  role                           = aws_iam_role.lambda_execution_role_prod.arn
