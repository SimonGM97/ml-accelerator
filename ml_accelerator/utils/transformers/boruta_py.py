...
class BorutaPy(BaseEstimator, TransformerMixin):
    ...
    def _get_tree_num(self, n_feat):
        # Retrieve max_depth, default to 10 if None
        depth = self.estimator.get_params().get('max_depth', 10)
        if depth is None or depth <= 0:
            depth = 10  # Use a default depth if the provided depth is invalid

        # Ensure n_feat is positive
        if n_feat <= 0:
            raise ValueError(f"Number of features must be positive. Got: {n_feat}")

        # Adjust the formula to ensure valid values for n_estimators
        f_repr = 100  # Number of times a feature should be considered on average
        multi = ((n_feat * 2) / (np.sqrt(n_feat * 2) * depth))

        # Add a lower bound for n_estimators
        n_estimators = max(1, int(multi * f_repr))  # Ensure at least 1 tree

        # Log or raise error if n_estimators is invalid
        if n_estimators <= 0:
            raise ValueError(f"Calculated n_estimators is not valid: {n_estimators}")
